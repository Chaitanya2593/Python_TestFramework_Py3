...
class GmailBOClass(object):
    ...
    def log_into_gmail(self, gmail_uname, gmail_pwrd):
        webcommomn = WebCommonActions()
        driver = webcommomn.launch_googlechrome(gmail_url)
        time.sleep(3)
        gmllp = GmailLoginPageActionClass(driver)
        drvr = gmllp.login_automation_gmail_account(gmail_uname, gmail_pwrd)
        return drvr

    def get_verification_code_from_gmail(self, gmail_uname, gmail_pwrd, sf_sender_name):
        drvr1 = self.log_into_gmail(gmail_uname, gmail_pwrd)
    ...
    def save_attachment_content(self, target_filepath):
        ...
        drvr = self.log_into_gmail(gmail_uname, gmail_pwrd)
        gmllp = GmailLoginPageActionClass(drvr)
        ...
        gmllp.gmail_sign_out_html(drvr4)
